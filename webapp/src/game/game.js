...
async function boot({ scene, camera, renderer }) {
  ...
  let mode = "walk"; // "walk" | "sitstand" | "roll" | "kickL" | "kickR" | "crouch" | "groundpick" | "dance"
  let sitFlag = 0;
  // Beat-synced dance: the player owns the audio + beat clock; the policy
  // session is lazy-loaded the first time music starts. While a track is
  // playing the duck dances whenever it is otherwise idle, and WASD steals
  // it back to the walking policy — the frozen 61D contract is what makes
  // that hot-swap safe mid-track.
  ...
  function buildObs() {
    ...
    cmd.fill(0, 0, 3);
    // Body slots cmd[7..12] belong to the dance policy (amplitudes); every
    // other policy was trained against zero-padding there, so they are
    // rewritten every step rather than left to persist across mode swaps.
    cmd.fill(0, 7, CMD_SIZE);
    if (mode === "sitstand") {
      cmd[0] = sitFlag;
      ...
      cmd[0] = Math.cos(a);
      cmd[1] = Math.sin(a);
    } else if (mode === "dance" && dancer.playing) {
      // Beat clock in the twist slots, straight off the audio hardware
      // clock (drift-free by construction); amplitudes in the body slots
      // exactly as microduck_dance.beat_source.BeatCommandSource lays
      // them out: [x, sway, bob, roll, pitch, yaw] with x/roll/pitch 0.
      const a = 2 * Math.PI * dancer.barPhase();
      cmd[0] = Math.cos(a);
      cmd[1] = Math.sin(a);
      cmd[2] = dancer.tempoNorm();
      cmd[8] = DANCE_SWAY;
      cmd[9] = DANCE_BOB;
      cmd[12] = DANCE_YAW;
    } else {
    ...
    // Dance zeroes them too: the policy was trained with a neutral head
    // command (BeatCommandSource.head_pose = 0) — the swing is its own.
    const gpZero = mode === "groundpick" || mode === "dance" || recovery !== null;
  ...
  async function controlStep() {
    ...
    if (death === "exploded") {
      ...
      if (recovery.state === "fallen") {
        ...
        if (recovery.uprightSteps >= RECOVER_UPRIGHT_STEPS) {
          recovery = null;
          // Back to dancing if the track is still going — the arbitration
          // block would do this anyway, this just skips the idle debounce.
          mode = dancer.playing && loco === "legs" && sessions.dance ? "dance" : "walk";
          lastAction.fill(0);
          syncButtons();
      ...
      const recoverable = loco === "legs" && (mode === "walk" || mode === "dance") &&
    ...
    // Dance <-> walk arbitration while a track plays (legs only). Movement
    // input steals the duck back to the walker instantly; half a second of
    // idle hands it to the dance policy. setMode carries all the guards
    // (one-shots, recovery, sit hand-backs), so a blocked swap just waits.
    if (dancer.playing && loco === "legs" && !inputLocked) {
      const c = controller.getCommand();
      const moving = Math.abs(c[0]) > 0.02 || Math.abs(c[1]) > 0.02 || Math.abs(c[2]) > 0.02;
      if (mode === "dance") {
        danceIdleSteps = 0;
        if (moving || headMode) setMode("walk");
      } else if (mode === "walk" && !headMode && postKickLock === 0 && !recovery && !standTimer) {
        danceIdleSteps = moving ? 0 : danceIdleSteps + 1;
        if (danceIdleSteps >= DANCE_IDLE_STEPS && sessions.dance) setMode("dance");
      } else {
        danceIdleSteps = 0;
      }
    } else if (mode === "dance") {
      // Track ended or was stopped: hand back to the walker.
      danceIdleSteps = 0;
      setMode("walk");
    }
  ...
  // ── Music / dance surface ────────────────────────────────────────────
  let dancePromise = null;
  const ensureDance = () => {
    dancePromise ??= ort.InferenceSession.create(signed(POLICIES.dance), sessionOpts)
      .then((s) => { sessions.dance = s; })
      .catch((err) => { dancePromise = null; throw err; });
    return dancePromise;
  };
  ...
  async function startDance(file) {
    try {
      setDanceStore({ state: "analyzing", title: file.name, note: null });
      const [prep] = await Promise.all([dancer.load(file), ensureDance()]);
      const notes = [];
      ...
      dancer.stop();
      setDanceStore({ state: "error", note: err?.message || String(err) });
    }
  }
